...
if ( ! function_exists( 'expiry_date_for_points' ) ) {
	...
	function expiry_date_for_points( $checkpoints = '' ) {
		...
		} else if ( 'yes' == get_option( 'rs_point_expiry_activated' ) && ! empty( get_option( 'rs_point_to_be_expire' ) ) ) {
...
if ( ! function_exists( 'order_total_in_order_detail' ) ) {
	...
	function order_total_in_order_detail( $total, $order ) {
		...
		if ( '2' === get_option('rs_pricing_type_global_level') ) {
			$tax_total_points	= redeem_point_conversion( $tax_total , $user_id );
			$TotalPoints		= $fee_total + $tax_total_points;
		} else {
			$TotalPoints	= redeem_point_conversion( $fee_total , $user_id ) ;
		}
