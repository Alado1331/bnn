...
if ( ! class_exists( 'FP_Rewardsystem_Frontend_Ajax' ) ) {
	...
	class FP_Rewardsystem_Frontend_Ajax {
		...
		public function rs_send_mail_for_referral_link() {
			...
			try {
				...
				if ( '' !== $error ) {
					throw new exception( __( 'Invalid Email!', 'rewardsystem' ) );
				}

				wp_send_json_success( __( 'Mail Shared Successfully!', 'rewardsystem' ) );
			} catch ( Exception $e ) {
				wp_send_json_error( array( 'error' => $e->getMessage(), 'ids' => $error ) );
			}
		}
			...
			try {				
				...
				if ( srp_check_is_array( $AppliedCoupons ) ) {
					...
					if ( in_array( $Redeem , $AppliedCoupons ) || in_array( $AutoRedeem , $AppliedCoupons ) ) {
						...
						if ( '5' == get_option( 'rs_show_hide_redeem_field' )) {
							$coupon_field	 = 'hide' ;
						}
				...
				if ( srp_check_is_array( $redeem_field ) ) {
				...
				} else if ( '2' === get_option( 'rs_show_hide_redeem_field' ) ) {
					$coupon_field	 = 'hide' ;
					if ( 2 == $HideRedeemField && check_if_discount_applied() ) {
						$cart_redeem 	 = 'hide' ;
						$checkout_redeem = 'hide' ;
				...
				} else if ('3' === get_option( 'rs_show_hide_redeem_field' ) ) {
					$coupon_field 	 = 'show' ;
					$cart_redeem 	 = 'hide' ;
					$checkout_redeem = 'hide' ;
				} else if ('4' == get_option( 'rs_show_hide_redeem_field' ) ) {
					$coupon_field 	 = 'hide' ;
					$cart_redeem 	 = 'hide' ;
					$checkout_redeem = 'hide' ;
				} else {
