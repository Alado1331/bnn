...
if ( ! class_exists( 'RSRedeemingFrontend' ) ) {
	...
	class RSRedeemingFrontend {
									...
									<div class="fp_apply_reward">
																				...
																				if ($predefined_option_values) : 
																					...
																					<div class="rs-predefined-button-wrapper">
																						<label><?php echo wp_kses_post(get_option('rs_redeeming_predefined_points_selection_label', 'Points Selection')); ?></label>
																						<select>
																							<option value="0"><?php echo wp_kses_post(get_option('rs_redeeming_predefined_choose_option_label', 'Select the Points')); ?></option>
										...
										<?php $placeholder = '1' == get_option( 'rs_show_hide_redeem_placeholder' ) ? get_option( 'rs_redeem_field_placeholder' ) : '' ; ?>
																				...
																				if ($readonly) :
																					?>
																					 readonly="readonly" <?php endif; ?> type="text" placeholder="<?php echo wp_kses_post($placeholder) ; ?>" value="" name="rs_apply_coupon_code_field">
									...
									<form name="checkout_redeeming" class="checkout_redeeming" method="post">
										<div class="fp_apply_reward">
																					...
																					if ($predefined_option_values) : 
																						...
																						?>
																						<div class="rs-predefined-button-wrapper">
																							<label><?php echo wp_kses_post(get_option('rs_redeeming_predefined_points_selection_label_checkout', 'Points Selection')); ?></label>
																							<select>
																								<option value="0"><?php echo wp_kses_post(get_option('rs_redeeming_predefined_choose_option_label_checkout', 'Select the Points')); ?></option>
																								<?php foreach ($predefined_option_values as $predefined_option_value) : ?>
																									<option value="<?php echo esc_attr($predefined_option_value); ?>"><?php echo esc_html($predefined_option_value); ?></option>
																								<?php endforeach; ?>
																							</select>
		...
		public static function auto_redeeming( $UserId, $Points ) {
			...
			if ( !rs_check_maximum_points_restriction_per_day($UserId)) {
			...
			if ( !rs_check_maximum_points_restriction_per_day($UserId, $CouponValue)) {
				wc_add_notice( str_replace('[max_points]', get_option('rs_maximum_redeeming_per_day_restriction'), get_option( 'rs_maximum_redeeming_per_day_error' , 'You are allowed to redeem a maximum of [max_points] points per day.') ), 'error' ) ;
				return ;
			}
		...
		public static function redeem_point_for_user() {
			...
			if ( isset( $_REQUEST[ 'rs_apply_coupon_code' ] ) || isset( $_REQUEST[ 'rs_apply_coupon_code1' ] ) || isset( $_REQUEST[ 'rs_apply_coupon_code2' ] ) ) {
				...
				if ( !rs_check_maximum_points_restriction_per_day($UserId, $redeeming_value)) {
					wc_add_notice(str_replace('[max_points]', get_option('rs_maximum_redeeming_per_day_restriction'), get_option( 'rs_maximum_redeeming_per_day_error' , 'You are allowed to redeem a maximum of [max_points] points per day.') ), 'error' ) ;
					return ;
				}
		...
		public static function button_type_redeem_field_in_cart_and_checkout() {
			...
			if ( '' !== $MinCartTotalToRedeem || '' !== $MaxCartTotalToRedeem ) {
				...
				} else if ( '' !== $MaxCartTotalToRedeem && $CartTotal >= $MaxCartTotalToRedeem ) {
					if ( '1' == get_option( 'rs_show_hide_maximum_cart_total_error_message' ) ) {
				...
				} else if ( $CartTotal >= $MinCartTotalToRedeem && $CartTotal <= $MaxCartTotalToRedeem ) {
		...
		public static function button_type_redeem_field() {
			...
			<form method="post" class="<?php echo esc_attr($FormClass) ; ?> woocommerce-info">
				...
				<div class="<?php echo esc_attr($DivClass) ; ?>"><?php echo wp_kses_post($ButtonMsg) ; ?>
					...
					<button id='mainsubmi' class="<?php echo esc_attr(get_option( 'rs_extra_class_name_apply_reward_points' ) ); ?>" type="submit" name="rs_apply_coupon_code1"><?php echo wp_kses_post(get_option( 'rs_redeem_field_submit_button_caption' ) ); ?></button>
				</div>
			</form>
			<?php
		}
		...
		public static function srp_get_maximum_redeem_points_based_on_product_total( $notice = false ) {
			...
			if ($notice) {
				return array( 'points' => array_sum( $redeem_points ), 'error' => $error );
			}
