	...
	class RSPointPriceFrontend {
		...
		public static function point_price_value( $product_price, $item, $PointPriceType, $IndividualPointsForProduct ) {
			...
			if ( '1' == $PointPriceType ) { //Currency & Point Pricing
				...
				$product_price	= str_replace( $separator , '' , $ProductPriceToDisplay ) ;
			}
		...
		public static function subtotal_in_cart( $CartSubTotal, $compound, $CartObj ) {
			...
			if ( ! empty( $OnlyPointPriceAmnt ) ) {
				...
				$separator    = get_option( 'rs_separator_for_point_price');
				$CartSubTotal = str_replace( $separator , '' , $CartSubTotal ) ;
				return $CartSubTotal ;
			}
			return $CartSubTotal ;
		}
		...
		public static function total_in_cart_with_shipping_and_tax( $price ) {
			...
			if ( '2' === get_option('rs_pricing_type_global_level') ) {
				...
				$FeeAmnt	  = $FeeAmnt + $tax_total_points;
				$TotalPoint	  = $FeeAmnt ;
			} else {
				$TotalPoint	  = redeem_point_conversion( $FeeAmnt , get_current_user_id() ) ;
			}

			$CartTotalToDisplay	  = display_point_price_value( $TotalPoint, true ) ;
			...
			if ( in_array( 'yes' , $EnablePointPriceforSimple ) || in_array( '1' , $EnablePointPriceForVariable ) || in_array( '1' , $EnablePointPriceValue ) ) {
				if ( in_array( '2' , $PointPriceType ) ) {
					...
					$separator	= get_option( 'rs_separator_for_point_price');
					$DisplayTotal	= str_replace( $separator , '' , $DisplayTotal ) ;
					return $DisplayTotal ;
