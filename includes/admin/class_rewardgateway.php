...
function init_reward_gateway_class() {
	...
	function gateway_points( $order_id ) {
		...
		$Points			= $order->get_total() ;
		$tax_total		= $order->get_total_tax();

		if ( '2' === get_option('rs_pricing_type_global_level') ) {
			$tax_total_points = redeem_point_conversion( $tax_total , $UserId );
			$redeemedpoints		= $Points + $tax_total_points ; 
		} else {
			$redeemedpoints	= redeem_point_conversion( $Points , $UserId );
		}
